<template>
    <div>
        <div class="flex justify-between items-center">
            <h3 class="text-xl font-bold dark:text-purple-400 animate-content-slide-in" style="animation-delay: 0.2s;">
                {{ playlist.name }}</h3>
            <div class="animate-content-slide-in" style="animation-delay: 0.2s;">
                <button @click="deletePlaylist(playlist.id)"
                    class="bg-red-500 hover:bg-red-600 text-white font-bold py-1 px-2 rounded  mr-2 dark:bg-red-700 dark:hover:bg-red-800"
                    style="animation-delay: 0.4s;">
                    Delete
                </button>
                <button @click="toggleSongs()"
                    class="bg-purple-500 hover:bg-purple-600 text-white font-bold py-1 px-2 rounded  dark:bg-purple-700 dark:hover:bg-purple-800"
        ...
        <ul v-if="selectedPlaylistId === playlist.id" class="mt-4">
            <li v-for="song in playlistSongs" :key="song.id"
                class="flex justify-between items-center p-2  rounded mt-2 dark:bg-gray-700 dark:text-gray-300 animate-content-slide-in"
                style="animation-delay: 0.2s;">
                <span>{{ song.title }} - {{ song.artist }}</span>
                <button @click="removeSongFromPlaylist(playlist.id, song.id)"
                    class="bg-orange-500 hover:bg-orange-600 text-white font-bold py-1 px-2 rounded transition-colors duration-300 dark:bg-orange-700 dark:hover:bg-orange-800 animate-content-slide-in"
                    style="animation-delay: 0.4s;">
                    Remove
                </button>
            </li>
        ...
        <div class="mt-4">
            <h4 class="text-lg font-semibold mb-2 dark:text-gray-300 animate-content-slide-in"
                style="animation-delay: 0.2s;">Add Song to Playlist</h4>
            ...
            <button @click="addSongToPlaylist(playlist.id)"
                class="bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-4 rounded-full transition-colors duration-300 mt-2 dark:bg-green-700 dark:hover:bg-green-800 animate-content-slide-in"
                style="animation-delay: 0.6s;">
                Add
            </button>
        </div>
