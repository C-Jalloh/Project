<template>
  <form @submit.prevent="submit" class="space-y-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div class="space-y-1">
        <label class="glass-label">Director</label>
        <input 
      ...
      <div class="space-y-1">
        <label class="glass-label">Duration (min)</label>
        <input 
    ...
    <div class="space-y-1">
      <label class="glass-label">Movie Poster</label>
      <div class="relative">
        <input 
          type="file"
          @change="onFileChange"
    ...
    <div class="pt-4 flex justify-end">
      ...
      >
        ...
        <span v-else>{{ isEdit ? 'Update Movie' : 'Create Movie' }}</span>
      </button>
    </div>
  </form>
</template>
...
// If editing, ensure genre and director are strings for the form inputs
if (props.isEdit && props.initialData) {
  if (Array.isArray(props.initialData.genres) && props.initialData.genres.length > 0) {
    formData.genre = props.initialData.genres.map(g => g.name).join(", ");
  }
  if (props.initialData.directorEntity) {
    formData.director = props.initialData.directorEntity.name;
  }
}
...
const submit = async () => {
  ...
  try {
    ...
    if (props.isEdit) {
      await apiClient.put(`/movies/${formData.id}`, payload);
    } else {
